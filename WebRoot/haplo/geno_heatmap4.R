...
cat("hcl calculation: ", end.time-start.time," secs.")
start.time <- end.time
...
cat("loaded genotypes : ", end.time-start.time," secs.")
start.time <- end.time
...
kgroup2color=c("brown","violet","blue","green","yellow", "orange","red", "#FF0000FF", "#FF4D00FF", "#FF9900FF", "#FFE500FF" ,"#CCFF00FF" ,"#80FF00FF", "#33FF00FF" ,"#00FF19FF")
...
#          MAsummary$kgroup<-cutree(hc, k=kgroups)
 ...
 #             MAsummary$kgroup<-cutree(hc, h=kheight)
    ...
    #  geom_text(aes(x = x, y = y, label = label, angle = -90, hjust = 0),size=0.5,  data= label(ddata)) +
        hdendo = hdendo + geom_text(aes(x = x, y = -0.1, label = label, angle = 0, hjust = 0, show.legend=F),size=0.3,  data= labs) 
       } else {
    ...
    #  coord_cartesian(xlim = c(10*offsety,10*(1-offsety))) +
     ...
     paste0("image written to ", pedfilename,'.hctree.',imageformat)
		...
		cat("phylotree drawing calculation: ", end.time-start.time," secs.")
		start.time <- end.time
...
for (ki in seq(0:kgroups)) {
      ...
      print(paste0("k=",k, ", rows=", nrow(MAsorted),",",nrow(POPsorted)))
        ...
        # y=Y, col=c(pop,kgroups), data=POPsorted , size=3, show.legend = F, shape="-", inherit.aes = F)
         ...
         h=h+ annotate(geom = "text", x=ncol(MAsorted)+0.1/cmpercolumn, y=nrow(MAsorted)-150, label="subpop", angle=-90, size=2, color="black") 

        if(kgroups>0) {
           h = h + geom_point( mapping  = aes(
           x= ncol(MAsorted)+ 0.4/cmpercolumn , # 3/ncol(MAsorted),
           y=Y, col=kgroup), data=POPsorted , size=3, show.legend = F, shape="-", inherit.aes = F) + annotate(geom = "text", x=ncol(MAsorted)+0.4/cmpercolumn , angle=-90, size=2, y=nrow(MAsorted)-100, label="K-group", color="black")
        }
