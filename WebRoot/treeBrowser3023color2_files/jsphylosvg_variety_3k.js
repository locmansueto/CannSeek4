	...
	object = function (parentNode) {
		...
		while (ch !== ')' && ch !== ',') {
			if (ch === ':'){
				...
				//node.uri = 'onClicknode("' +  ch.split('/')[1] + '")'; 
				//node.uri = node.name;
			} else {
				node.type = "label";
				node.name = string();
...
Smits.PhyloCanvas.NexmlParse.prototype = {
	...
	Rectangular : {
		bufferX			: 500, //300, //200, 			// Reduces the available canvas space for tree branches, allowing
										// for more space for the textual/charting components
		paddingX		: 8, //10,
		paddingY		: 15,  //20,
		bufferInnerLabels : 10, 		// Pixels
		bufferOuterLabels : 5, 			// Pixels
		minHeightBetweenLeaves : 8, //10,  	// Should probably set pretty low, as clipping may occur if it needs to be implemented		
	...
	barCharts : [],
		...
		*/
		mouseRollOver : function(params) {
			...
			params.textEl.attr["font-weight"]= 'bolder';
		},
		mouseRollOut : function(params) {
			params.node.edgeCircleHighlight.hide();
			//params.textEl.attr({ fill: '#000' });
			params.textEl.attr["font-weight"]= 'normal';
		},
		onClickAction : function(params) {
			
			 (function(window){
				 if(params.textEl.attrs.text.split('|')[1]=='')
					window.zAu.send(new window.zk.Event(window.zk.Widget.$('$winVariety'), "onUser", "name=" + params.textEl.attrs.text.split('|')[0]));
				 else	 
		        	window.zAu.send(new window.zk.Event(window.zk.Widget.$('$winVariety'), "onUser", "irisid=" + params.textEl.attrs.text.split('|')[1]));
		     })(parent);
			 
		},
		onClickStemAction : function(params) {
			
			// get all descendant nodes name
			var leafnodesirisids = [];
			var leafnodesnames = [];
			
			recursiveGetLeafNodes = function (node){
				...
				if(node.children.length > 0){
					...
					if(node.name){
						...
						if(nodenames[1]!='') {
							leafnodesirisids.push(nodenames[1]);
						} else
						{
							leafnodesnames.push(nodenames[0]);
						}
			...
			recursiveGetLeafNodes(params.node);
			
			var paramval = '';
			if( leafnodesirisids.length>0 && leafnodesnames.length>0)
				paramval = 'name=' + leafnodesnames.join() + '&irisid=' + leafnodesirisids.join();
			else if( leafnodesirisids.length>0 )
				paramval = 'irisid=' + leafnodesirisids.join();
			else if( leafnodesnames.length>0 )
				paramval = 'name=' + leafnodesnames.join();

			
			 (function(window){
					window.zAu.send(new window.zk.Event(window.zk.Widget.$('$winVariety'), "onUser", paramval )); 
		     })(parent);
	...
	recursiveCalculateNodePositions = function (node, positionX){
		...
		if(node.children.length > 0){
			...
			if(node.len){ // draw stem
				...
				y2 = y1;
				var drawStem = svg.draw(new Smits.PhyloCanvas.Render.Line(x1, x2, y1, y2));
				
				// add branch (stem) click handler here
				
				if(Smits.PhyloCanvas.Render.Parameters.onClickStemAction){
					Smits.Common.addRaphEventHandler(
						drawStem[0], 
						'click', 
						Smits.PhyloCanvas.Render.Parameters.onClickStemAction, 
						{ svg: svg, node: node, x: x2, y: y2, textEl: drawStem[0] }
					);				
				}
				...
				if(Smits.PhyloCanvas.Render.Parameters.mouseRollOver){
					Smits.Common.addRaphEventHandler(
						...
						{ svg: svg, node: node, x: x1, y: y2, textEl: drawStem[0] }
					);
				}
			...
			if(node.name){ // draw bootstrap values
				...
				/*
				var variety_color = 'black';
				var nodenames = node.name.split('/');
				if( nodenames.length>2 )
					{
						switch(nodenames[2]) {
							case 'AUS': variety_color='orange'; break;
							case 'TEMP': variety_color='pink'; break;
							case 'MAGIC': variety_color='violet'; break;
							case 'TROP': variety_color='green'; break;
							case 'IND': variety_color='blue'; break;
							case 'ARO': variety_color='silver'; break;
							case 'ADM': variety_color='aqua'; break;
							case 'JAP': variety_color='firebrick'; break;
							case 'NULL': variety_color='yellowgreen'; break;
						}
					}
				attr["fill"] =  variety_color;	
				*/
				attr["fill"] = 'black';
				svg.draw(
		...
		/* old names
						...
						case 'ind1.2.2': variety_color='#306600'; break;

						case 'ind1B': variety_color='aquamarine'; break;

						case 'ind2': variety_color='#336622'; break;
						case 'ind2.1': variety_color='#306600'; break;
						case 'ind2.2': variety_color='#507755'; break;

						case 'ind3': variety_color='#339910'; break;
						case 'ind3.1': variety_color='#339910'; break;
						case 'ind3.2': variety_color='#299506'; break;
						case 'ind3.2.1': variety_color='#a2cd5a'; break;
						case 'ind3.2.2': variety_color='#33cc77'; break;

						case 'trop1': variety_color='#4682b4'; break;
						case 'trop1.1': variety_color='#4f94cd'; break;
						case 'trop1.2': variety_color='#3163aa'; break;
		...
		*/
						...
						/*
						//v1
						case 'admx':  variety_color='#006666'; break;
						case 'aus':  variety_color='#CC7A52'; break;
						case 'trop/temp':  variety_color='#FF33CC'; break;
						case 'inax':  variety_color='#00CC99'; break;
						case 'temp':  variety_color='#0066FF'; break;
						case 'aro':  variety_color='#AD8533'; break;
						case 'trop':  variety_color='#FF3300'; break;
						case 'japx':  variety_color='#FFCC00'; break;
						case 'ind1':  variety_color='#009900'; break;
						case 'ind3':  variety_color='#99CC00'; break;
						case 'ind2':  variety_color='#718C64'; break;
						case 'indx':  variety_color='#A3FFA3'; break;
						case 'temp/trop':  variety_color='#6600CC'; break;
						
						case 'Basmati_sadri_Admx': variety_color='sandybrown'; break;
						case 'Intermediate_type_Temp': variety_color='gold'; break;
						case 'Intermediate_type_Japx': variety_color='palegoldenrod'; break;
						case 'Indica': variety_color='blue'; break;
						case 'Basmati_sadri': variety_color='peru'; break;
						case 'Japonica_Admx': variety_color='crimson'; break;
						case 'Temperate_japonica_Indi': variety_color='firebrick'; break;
						case 'Basmati_sadri_Arom': variety_color='burlywood'; break;
						case 'Indica_AusB': variety_color='dodgerblue'; break;
						case 'Temperate_japonica_Temp': variety_color='pink'; break;
						case 'Intermediate_type_Arom': variety_color='yellow'; break;
						case 'Temperate_japonica_Admx': variety_color='red'; break;
						case 'Tropical_japonica': variety_color='lightsalmon'; break;
						case 'Japonica': variety_color='orangered'; break;
						case 'Tropical_japonica_Trop': variety_color='palevioletred'; break;
						case 'Intermediate_type': variety_color='wheat'; break;
						case 'Temperate_japonica': variety_color='deeppink'; break;
						case 'Tropical_japonica_Admx': variety_color='fuchsia'; break;
						case 'Aus_boro_Arom': variety_color='greenyellow'; break;
						case 'Aus_boro_AusB': variety_color='darkolivegreen'; break;
						case 'Tropical_japonica_Temp': variety_color='hotpink'; break;
						case 'Aus_boro': variety_color='green'; break;
						case 'Indica_Indi': variety_color='aqua'; break;
						...
						*/
						...
						/*
						Basmati/sadri, Admx
						Intermediate type, Temp
						Intermediate type, Japx
						Indica
						Basmati/sadri
						Japonica, Admx
						Temperate japonica, Indi
						Basmati/sadri, Arom
						Indica, AusB
						Temperate japonica, Temp
						Intermediate type, Arom
						Temperate japonica, Admx
						Tropical japonica
						Japonica
						Tropical japonica, Trop
						Intermediate type
						Temperate japonica
						Tropical japonica, Admx
						Aus/boro, Arom
						Aus/boro, AusB
						Tropical japonica, Temp
						Aus/boro
						Indica, Indi
						*/
