...
#ICGRC
webappsdir='/usr/local/tomcat/webapps/cannseek/'
legendpath=paste0(webappsdir,'haplo/legend.png')
...
ec=paste0("python ", webappsdir, "haplo/rename-pednames.py ", pedfilename)
ec
system(ec)
ec=paste0("mv ",pedfilename," ",pedfilename,".orignames")
ec
system(ec)
ec=paste0("mv ",pedfilename,".newnames ", pedfilename)
ec
system(ec)
...
if(F){
#recode ped to 0,1,2
plink_command=paste0(plink_root,"plink --allow-extra-chr --ped ",shQuote(pedfilename)," --recodeA --out ",shQuote(pedfilename)," --map ", shQuote(mapfilename), " --a2-allele ", shQuote(mapfilename),".ref")
plink_command
...
# remove # in ped
cmd=paste0("cp ", pedfilename," ", pedfilename,".bak")
cmd
system(cmd)
cmd=paste0("sed -i 's/#//g' ", pedfilename)
cmd
system(cmd)

 # make BED
plink_command=paste0(plink_root,"plink --allow-extra-chr --ped ",shQuote(pedfilename)," --make-bed  --out ",shQuote(paste0(pedfilename,"out"))," --map ", shQuote(mapfilename), " --a2-allele ", shQuote(paste0(mapfilename,".ref")))
plink_command
ec = system(plink_command)

cmd=paste0("python ",webappsdir, "haplo/create-summary.py ", pedfilename)
cmd
system(cmd)
...
#A[1:4,1:5]
 ...
 if(T){
	...
	plink_command = paste0(plink_root, "plink --alow-extra-chr --distance 1-ibs allele-ct square gz  --ped ", shQuote(pedfilename), " --map ", shQuote(mapfilename), " --out " , shQuote(pedfilename))
	plink_command
...
for (ki in seq(0:kgroups)) {
      ...
      print(paste0("k=",k, ", rows=", nrow(MAsorted),",",nrow(POPsorted)))
        ...
        pop2color =  c("magenta","#00ff01","red","orange","red","cyan","blue","white","pink","darkred","darkgreen","darkblue","gray","darkgray","yellow","red")
        names(pop2color) = c("bldt","basal cannabis","drug","drug type feral","drug-type","hemp","hemp-type","na","nldt","type i","type ii","type iii","csativa","0","1","2")
        ...
        # y=Y, col=c(pop,kgroups), data=POPsorted , size=3, show.legend = F, shape="-", inherit.aes = F)
         ...
         y=Y, col=pop), data=POPsorted , size=10, show.legend = F, shape=pointshape, solid=T, inherit.aes = F)
        ...
        if(kgroups>0) {
           ...
           y=Y, col=kgroup), data=POPsorted , size=10, show.legend = F, shape=pointshape, solid=T, inherit.aes = F) + annotate(geom = "text", x=ncol(MAsorted)+0.4/cmpercolumn , angle=-90, size=2, y=nrow(MAsorted)-annotlabelyoffset2, label="K-group", color="black")
        }
