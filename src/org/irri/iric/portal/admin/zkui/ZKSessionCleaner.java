...
public class ZKSessionCleaner implements SessionCleanup {
	...
	public void cleanupOld(org.zkoss.zk.ui.Session zksession) throws Exception {
		...
		/*
		 * oraclesessionDao = (VOracleSessionsDAO)AppContext.checkBean(oraclesessionDao,
		 * "VOracleSessionsDAO"); Iterator<VOracleSessions> itOracleSession =
		...
		if (AppContext.isIRRILAN())
			...
			AppContext.debug("Cleaning onClose session " + zksession.getRemoteHost() + "  " + zksession.getRemoteAddr()
					+ "  id=" + ((HttpSession) zksession.getNativeSession()).getId() + " stostring="
					+ zksession.getNativeSession());
	...
	private org.hibernate.Session getSession() {
		...
		// entityManager = (EntityManager)AppContext.checkBean(entityManager,
		// "IRIC_Production");
