...
public class H5 implements java.io.Serializable {
	...
	public static void loadH5Lib() {
		...
		if ((s_libraryName != null) && (s_libraryName.length() > 0)) {
			...
			AppContext.debug((isLibraryLoaded ? "" : " NOT") + " successfully loaded from system property " + s_libraryName);
		}
			...
			if ((filename != null) && (filename.length() > 0)) {
				...
				if (h5dll.exists() && h5dll.canRead() && h5dll.isFile()) {
					try {
						...
						log.debug(filename);
						log.info((isLibraryLoaded ? "" : " NOT") + " successfully loaded via full path.");
						AppContext.debug((isLibraryLoaded ? "" : " NOT") + " successfully loaded via full path.");
					}
				} else {
					isLibraryLoaded = false;
		...
		if (!isLibraryLoaded) {
			try {
				...
				AppContext.debug((isLibraryLoaded ? "" : " NOT") + " successfully loaded from java.library.path");
			}
			AppContext.debug((isLibraryLoaded ? "" : " NOT") + " successfully loaded from system property " + s_libraryName);
		...
		if (!isLibraryLoaded) {
			...
			try {
				...
				if (AppContext.isWindows()) {
					...
					System.out.println(AppContext.getFlatfilesDir() + libpath + "/libjhdf5.so");
