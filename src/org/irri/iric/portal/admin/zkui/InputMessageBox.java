...
public class InputMessageBox extends Messagebox {
	 ...
	 * 
	 * @param message
	 *            the message to show before the input field.
	 * @param title
	 *            the title. If null, {@link WebApp#getAppName} is used.
	 * @return the object. NULL means that CANCEL, otherwise means an Object is
	 *         being passed.
	 * 
	 *         Note: if the event processing thread is disable, it always returns
	 *         null.
	 */
	public static final Object showQuestion(String message, String title) throws InterruptedException {
		setTemplate(_templ);
		Object object = null;
		if (showInput(message, title, Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, 0,
				null) == Messagebox.OK) {
			object = txtQuestion.getValue(); // TODO: Remember NULL means that CANCEL, otherwise means an Object is
												// being passed. I used StringUtils from commons lang to trim to empty
		}
	 ...
	 *            {@link #NO}, {@link #ABORT}, {@link #RETRY}, and {@link #IGNORE}.
	 *            If zero, {@link #OK} is assumed
	 * @param icon
	 *            one of predefined images: {@link #QUESTION}, {@link #EXCLAMATION},
	 *            {@link #ERROR}, {@link #NONE}, or any style class name(s) to show
	 *            an image.
	 * @param focus
	 *            one of button to have to focus. If 0, the first button will gain
	 *            the focus. One of {@link #OK}, {@link #CANCEL}, {@link #YES},
	 *            {@link #NO}, {@link #ABORT}, {@link #RETRY}, and {@link #IGNORE}.
	 * @param listener
	 *            the event listener which is invoked when a button is clicked.
	 *            Ignored if null. It is useful if the event processing thread is
	 *            disabled
	 *            ({@link org.zkoss.zk.ui.util.Configuration#enableEventThread}). If
	 *            the event processing thread is disable, this method always return
	 *            {@link #OK}. To know which button is pressed, you have to pass an
	 *            event listener. Then, when the user clicks a button, the event
	 *            listener is invoked. You can identify which button is clicked by
	 *            examining the event name
	 ...
	 * @since 3.0.4
	 */
	public static final int showInput(String message, String title, int buttons, String icon, int focus,
