...
if(kgroups>0 || kheight>0) {
    ...
    #  geom_text(aes(x = x, y = y, label = label, angle = -90, hjust = 0),size=0.5,  data= label(ddata)) +
       geom_text(aes(x = x, y = -0.1, label = label, angle = 0, hjust = 0),size=0.2,  data= label(ddata)) +
    ...
    #  coord_cartesian(ylim = c(nrow(MAsorted)*offsety,nrow(MAsorted)*(1-offsety))) +
       coord_cartesian(xlim = c(ncol(MAsorted)*offsety,ncol(MAsorted)*(1-offsety)), ylim = c(10*offsety,10*(1-offsety))) +
    ...
    # hdendo  <- ggdendrogram(hcdata, rotate=TRUE, size=2) + theme_dendro()
      ...
      ggsave( paste0(pedfilename,'.hctree.',imageformat), plot=hdendo,  device=imageformat, hei=30, wi=4, dpi=800)
      paste0("image written to ", pedfilename,'.hctree.',imageformat)
        ...
        # SELECT BY GROUP
        MAsorted$kgroup<-MAsummary$kgroup
        POPsorted$kgroup<-MAsummary$kgroup
        ...
        for(i in seq(1:nrows)) {
          if(i==nrows) break;
          if( (MAsummary$kgroup[i]-MAsummary$kgroup[i+1]) != 0 ) {
            newgroup[ik]=i
            prevgroup[ik]=MAsummary$kgroup[i]
            ik=ik+1
          }
        }
        print("newgroup")
...
for (ki in seq(0:kgroups)) {
      ...
      print(paste0("k=",k, ", rows=", nrow(MAsorted),",",nrow(POPsorted)))
       ...
       if(!GENOMIC_COORD) {
        ...
        names(pop2color) = c("ind1A","ind1B","ind2","ind3","trop1","trop2","trop","subtrop","aro","temp","aus","admix","japx","indx", "0","1","2","k1","k2","k3","k4","k5","k6","k7")
        ...
        # y=Y, col=c(pop,kgroups), data=POPsorted , size=3, show.legend = F, shape="-", inherit.aes = F)
         ...
         h = h + geom_point( mapping  = aes(
         x= ncol(MAsorted)+1/(2*ncol(MAsorted)),
         y=Y, col=pop), data=POPsorted , size=3, show.legend = F, shape="-", inherit.aes = F)

         #h=h+geom_point( aes(size=0))


         h=h+ annotate(geom = "text", x=ncol(MAsorted)+1/(2*ncol(MAsorted)), y=nrow(MAsorted)-150, label="subpop", angle=-90, size=2, color="black") 

        if(kgroups>0) {
           h = h + geom_point( mapping  = aes(
           x= ncol(MAsorted)+3/ncol(MAsorted),
           y=Y, col=kgroup), data=POPsorted , size=3, show.legend = F, shape="-", inherit.aes = F) + annotate(geom = "text", x=ncol(MAsorted)+3/ncol(MAsorted), angle=-90, size=2, y=nrow(MAsorted)-100, label="K-group", color="black")
        }
