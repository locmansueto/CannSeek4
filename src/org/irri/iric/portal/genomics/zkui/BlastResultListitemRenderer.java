...
public class BlastResultListitemRenderer implements ListitemRenderer {
	...
	public void render(Listitem item, Object data, int arg2) throws Exception {
		...
		/*
		 * Listcell lc = new Listcell (); //new
		 * Label(locus.getUniquename()).setParent(row); new
		 * Label(locus.getQacc()).setParent(lc); new
		 * Label(locus.getQstart().toString()).setParent(lc); new
		 * Label(locus.getQend().toString()).setParent(lc); new
		 * Label(locus.getSacc()).setParent(lc); new
		 * Label(locus.getSstart().toString()).setParent(lc);
		 * 
		 * 
		 * new Label(locus.getSend().toString()).setParent(lc);
		 * 
		 * String sstrand = "N/A"; if(locus.getSstrand()!=0)
		 * sstrand=locus.getSstrand().toString(); new Label(sstrand).setParent(lc); new
		 * Label( locus.getMatches() + "/" + locus.getAlignmentLength() + " (" +
		 * locus.getPercentMatches() + "%)").setParent(lc); new
		 * Label(locus.getEvalue().toString()).setParent(lc); lc.setParent(item);
		 */
