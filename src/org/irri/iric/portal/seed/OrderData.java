...
public class OrderData {
	...
	public OrderData() {
		...
		// (String accession, String varname, String stockType, Integer gram, Double
		// pricePerGram)

		/*
		 * List allSeeds=new ArrayList(); List l=new ArrayList();
		 * 
		 * allSeeds.add(new Seed("IRGC 7474","klii","BL",0,1.0)); allSeeds.add(new
		 * Seed("IRGC 4556","ghgfh","BL",0,1.0)); allSeeds.add(new
		 * Seed("IRGC 6763","xc","PL",0,1.0)); allSeeds.add(new
		 * Seed("IRGC 2456","jhfgyt","PL",0,1.0)); allSeeds.add(new
		 * Seed("IRGC 7563","jhhklk","BL",0,1.0)); allSeeds.add(new
		 * Seed("IRGC 12354","nnvn","BL",0,1.0)); addVarietyList(allSeeds);
		 */
		...
		for (Object s : varlist) {
			if (s instanceof Seed) {
				...
				if (v.getAccession() != null && v.getAccession().startsWith("IRGC")) {
					...
					continue;
				if (setAccessions.contains(v.getAccession()))
					continue;
				allSeeds.add(v);
				...
				if (category.equals("pub")) {
					seed = new Seed(v.getAccession(), v.getName(), "PL", grams,
							seedpricedao.getPricePer10gram("PL", seedpricedao.getCountryCategory(country)),
							seedpricedao.getMaxFreeGram(seedpricedao.getCountryCategory(country)));
				} else if (category.equals("irri")) {
					seed = new Seed(v.getAccession(), v.getName(), "PL", grams,
							seedpricedao.getPricePer10gram("PL", "irri"), 0.0);
				} else
					seed = new Seed(v.getAccession(), v.getName(), "PL", grams,
							seedpricedao.getPricePer10gram("PL", "priv"), 0.0);
	...
	public void setRequestorInfo(String name, String institution, String address1, String address2, String country,
			String category, String email, String phone, String postalcode) {
		this.name = name;
		this.institution = institution;
		this.address1 = address1;
		this.address2 = address2;
	...
	public void setAuthorizedInfo(String acceptanceType, String name, String position, String address, String phone,
			String email) {
		this.autname = name;
		this.position = position;
